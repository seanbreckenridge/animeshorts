...
def navbar(active, **kwargs):
    ...
    with tag('nav', klass='navbar navbar-expand-lg navbar-dark bg-dark navbar-static-top container'):
        # hamburger menu
        with tag('button', ('class', 'navbar-toggler'), ('data-toggle', 'collapse'), ('data-target', '#collapse_navbar_target')):
        ...
        with tag('div', klass='collapse navbar-collapse', id='collapse_navbar_target'):
            with tag('ul', klass='navbar-nav nav-pills dark-blue'):
                with tag('li', klass='nav-item{}'.format(" active" if active==constants.LIST_TAB else "")):
                    with tag('a', ('class', 'nav-link px-2'), ('style', '{}'.format("background-color: #385289;" if active==constants.LIST_TAB else "")), ('href', '/')):
                        text(constants.LIST_TAB)
                with tag('li', klass='nav-item{}'.format(" active" if active==constants.PEOPLE_TAB else "")):
                    with tag('a', ('class', 'nav-link px-2'), ('style', '{}'.format("background-color: #385289;" if active==constants.PEOPLE_TAB else "")), ('href', '/people')):
                        text(constants.PEOPLE_TAB)
            if active == constants.LIST_TAB:
                with tag('ul', klass='navbar-nav'):
                    with tag('li', klass='nav-item'):
                        with tag('a', klass='mx-2', style="color: white; position: relative; top: 0.1rem;"):
                            text("Order By:")
                        with tag('div', ('class', 'btn-group ml-1'), ('role', 'group'), ('id', 'orderchoice')):
                            with tag('button', ('class', 'btn btn-secondary{}'.format(" active" if sorttab==constants.order.REC else ""))):
                                text('Recommendation')
                            with tag('button', ('class', 'btn btn-secondary{}'.format(" active" if sorttab==constants.order.DATE else ""))):
                                text('Date Added')
